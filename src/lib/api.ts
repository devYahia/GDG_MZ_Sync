...
export type SimulationMilestone = Milestone
...
export async function generateSimulation(data: GenerateSimulationRequest): Promise<GenerateSimulationResponse> {
    ...
    if (!res.ok) {
        ...
        console.error("[API Error] generateSimulation failed:", err, res.status, res.statusText)
...
export async function postProjectChat(data: ProjectChatRequest): Promise<{ reply: string }> {
    ...
    if (!res.ok) {
        const err = await res.json().catch(() => ({}))
        console.error("[API Error] postProjectChat failed:", err, res.status, res.statusText)
