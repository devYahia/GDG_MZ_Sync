...
export function ProgressSnapshot({
    currentLevel, nextLevel, xp, xpPercent, streakDays, credits, earnedBadges, skillAverages,
    ...
    return (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="rounded-2xl border border-white/5 bg-white/[0.03] p-5">
                <p className="text-sm font-medium mb-2">Skill Profile</p>
                {radarData ? (
                    <div className="h-36">
                ...
                {earnedBadges.length === 0 ? (
                    ...
                    <div className="flex flex-wrap gap-1.5">
                        {earnedBadges.slice(0, 8).map((badge) => (
                            <span key={badge.slug} title={badge.title} className={cn("inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium", RARITY_COLORS[badge.rarity] ?? RARITY_COLORS.common)}>
                                {badge.title}
                            </span>
                        ))}
                        {earnedBadges.length > 8 && <span className="text-xs text-muted-foreground self-center">+{earnedBadges.length - 8} more</span>}
                    </div>
                )}
            </div>
        </div>
